# Nexus File Manager — User & Architecture Guide

**Nexus File Manager** is the high-performance, native Qt6 file explorer integrated directly into **Cortex Cleaner**. Built for responsive directory navigation, multi-source staging, real-time transfer telemetry, smart scaffolding, and deep safety guards.
...
- **Tab Management**:
  ...
  - Automatic session restoration preserves all open tabs across app launches.
...
- **Instant Search (`Ctrl+F` / `F3`)**: Real-time regex and glob filtering directly inside the active directory with highlight matches.
