...
def run_smoke_test() -> int:
    ...
    test(
        "Paint size",
        lambda: (_ for _ in ()).throw(Exception(f"{pm.width()}x{pm.height()}")) if pm.width() < 100 else None,
    )
    ...
    test(
        "PreviewPane dir",
        lambda: p.show_entry({"name": "Desktop", "isDir": True, "path": os.path.expanduser("~/Desktop")}),
    )
    ...
    test(
        ...
        lambda: (_ for _ in ()).throw(Exception("empty folder tree")) if w.folder_tree.model.rowCount() == 0 else None,
    )
    test(
        "Status text",
        lambda: (
            (_ for _ in ()).throw(Exception(w.status_items.text())) if "items" not in w.status_items.text() else None
        ),
    )
    ...
    test(
        "Tab count >= 2",
        lambda: (_ for _ in ()).throw(Exception(f"{w.tabbar.count()}")) if w.tabbar.count() < 2 else None,
    )
    ...
    test(
        "Nav path correct",
        lambda: (_ for _ in ()).throw(Exception(w._tab()["path"])) if "Downloads" not in w._tab()["path"] else None,
    )
    ...
    test(
        "QSS has all selectors",
        lambda: (
            (_ for _ in ()).throw(Exception("missing selectors"))
            if "#NexusRoot" not in DARK_QSS or "#Status" not in DARK_QSS
            else None
        ),
    )
    ...
    test(
        "Tab count after close",
        lambda: (_ for _ in ()).throw(Exception(f"{w.tabbar.count()}")) if w.tabbar.count() < 1 else None,
    )

    # 19. Quick access click
    test(
        "Quick access item",
        lambda: w.quick_list.item(0).data(w.quick_list.item(0).data(256) is not None and 256 or 256),
    )

    # 20. Dual pane
    test("Dual pane toggle on", lambda: w._toggle_dual_pane())
    test(
        "Dual pane flag", lambda: (_ for _ in ()).throw(Exception(f"dual={w._dual_pane}")) if not w._dual_pane else None
    )
    test(
        "Right stack visible",
        lambda: (_ for _ in ()).throw(Exception("hidden")) if not w._right_stack.isVisible() else None,
    )
    ...
    test(
        "Right pane has rows",
        lambda: (_ for _ in ()).throw(Exception(f"0 rows")) if w._right_model.rowCount() == 0 else None,
    )
    test("Dual pane toggle off", lambda: w._toggle_dual_pane())
    test(
        "Dual pane off flag", lambda: (_ for _ in ()).throw(Exception(f"dual={w._dual_pane}")) if w._dual_pane else None
    )
    ...
    test(
        "QuickLook object",
        lambda: (_ for _ in ()).throw(Exception("missing")) if not hasattr(w, "_quicklook") else None,
    )

    # 22. Bulk Rename
    test(
        "BulkRename dialog class",
        lambda: (_ for _ in ()).throw(Exception("missing")) if BulkRenameDialog is None else None,
    )

    # 23. Folder Size Calculator
    test(
        "FolderSizeCalculator",
        lambda: (_ for _ in ()).throw(Exception("missing")) if not hasattr(w, "_folder_sizes") else None,
    )

    # 24. Color Tags
    test(
        "ColorTagManager",
        lambda: (_ for _ in ()).throw(Exception("missing")) if not hasattr(w, "_color_tags") else None,
    )
    test("Color tag set", lambda: w._color_tags.set_tag("test_path", "red"))
    test(
        "Color tag get",
        lambda: (_ for _ in ()).throw(Exception("wrong")) if w._color_tags.get_tag("test_path") != "red" else None,
    )
    ...
    test(
        "SmartFolderManager",
        lambda: (_ for _ in ()).throw(Exception("missing")) if not hasattr(w, "_smart_folders") else None,
    )
    test("Smart folder add", lambda: w._smart_folders.add("Test", "~", "*.txt"))
    test(
        "Smart folder list",
        lambda: (_ for _ in ()).throw(Exception("empty")) if len(w._smart_folders.list_all()) == 0 else None,
    )
    test("Smart folder sidebar refresh", lambda: w._refresh_smart_folders())
    test(
        "Smart folder sidebar", lambda: (_ for _ in ()).throw(Exception("empty")) if w.smart_list.count() == 0 else None
    )

    # 26. Command Palette
    test(
        "CommandPalette class", lambda: (_ for _ in ()).throw(Exception("missing")) if CommandPalette is None else None
    )
    test(
        "CommandPalette instance",
        lambda: (_ for _ in ()).throw(Exception("missing")) if not hasattr(w, "_palette") else None,
    )
    test(
        "CommandPalette has actions",
        lambda: (_ for _ in ()).throw(Exception("0 actions")) if len(w._palette._actions) == 0 else None,
    )
    ...
    # 27. Terminal
    test(
        "TerminalWidget class", lambda: (_ for _ in ()).throw(Exception("missing")) if TerminalWidget is None else None
    )
    test(
        "TerminalWidget instance",
        lambda: (_ for _ in ()).throw(Exception("missing")) if not hasattr(w, "terminal_panel") else None,
    )
    test("Terminal toggle", lambda: w._toggle_terminal())
    test(
        "Terminal visible",
        lambda: (_ for _ in ()).throw(Exception("hidden")) if not w.terminal_panel.isVisible() else None,
    )
    test("Terminal toggle off", lambda: w._toggle_terminal())

    # 28. Job Queue
    test(
        "JobQueueWidget class", lambda: (_ for _ in ()).throw(Exception("missing")) if JobQueueWidget is None else None
    )
    test(
        "JobQueueWidget instance",
        lambda: (_ for _ in ()).throw(Exception("missing")) if not hasattr(w, "_job_queue") else None,
    )
    test("Job add", lambda: w._job_queue.add_job("test job", 100))
    test(
        "Job update",
        lambda: w._job_queue.update_job(0, 50, "file.txt") if hasattr(w._job_queue, "update_job") else None,
    )
    test("Job complete", lambda: w._job_queue.complete_job(0) if hasattr(w._job_queue, "complete_job") else None)

    # 29. Duplicate Finder
    test(
        "DuplicateFinderDialog class",
        lambda: (_ for _ in ()).throw(Exception("missing")) if DuplicateFinderDialog is None else None,
    )

    # 30. Properties Dialog
    test(
        "PropertiesDialog class",
        lambda: (_ for _ in ()).throw(Exception("missing")) if PropertiesDialog is None else None,
    )

    # 31. Session persistence
    test(
        "Has save_session",
        lambda: (_ for _ in ()).throw(Exception("missing")) if not callable(getattr(w, "save_session", None)) else None,
    )
    test(
        "Has restore_session",
        lambda: (
            (_ for _ in ()).throw(Exception("missing")) if not callable(getattr(w, "restore_session", None)) else None
        ),
    )
    ...
    test(
        ...
        lambda: (_ for _ in ()).throw(Exception("wrong mode")) if w.table.selectionMode().value != 3 else None,
    )
    ...
    test(
        "BulkRename has modes",
        lambda: (_ for _ in ()).throw(Exception("missing")) if not hasattr(BulkRenameDialog, "__init__") else None,
    )
