...
@dataclass
class BrokenLink:
    """Base record for one broken link: path, missing target, size, timestamps, and repair state."""
...
class BrokenSymlink(BrokenLink):
    """Broken symlink with an absolute/relative-target flag."""

    is_absolute: bool = False

    def __post_init__(self):
        """Set link_type after init to the subclass link kind."""
        self.link_type = "symlink"


@dataclass
class BrokenShortcut(BrokenLink):
    """Broken Windows .lnk shortcut with working-dir, arguments, and icon metadata."""
...
class BrokenRegistryRef(BrokenLink):
    """Registry value whose referenced file path no longer exists."""
...
@dataclass
class RepairResult:
    """Outcome of one repair attempt: success, new target, backup, and error."""
...
@dataclass
class RepairOutcome:
    """Outcome of one cleanup action in repair(): path, action taken, success, and detail."""
    ...
    for item in items:
        ...
        try:
            ...
            if isinstance(item, BrokenRegistryRef) or link_type == "registry_ref":
                outcomes.append(
                    RepairOutcome(path=path, action="excluded", ok=False, detail="registry refs require manual review")
                )
                continue
            ...
            if isinstance(item, BrokenShortcut) or link_type == "shortcut":
                if not path.is_file() and not path.is_symlink():
                    outcomes.append(
                        RepairOutcome(path=path, action="skipped", ok=False, detail="shortcut file not found")
                    )
                    continue
                if dry_run:
                    outcomes.append(
                        RepairOutcome(
                            path=path, action="recycle shortcut", ok=True, detail="planned: move .lnk to Recycle Bin"
                        )
                    )
                    continue
                if use_trash:
                    if send2trash is None:
                        outcomes.append(
                            RepairOutcome(
                                path=path, action="failed", ok=False, detail="send2trash not installed; cannot recycle"
                            )
                        )
                        continue
                    send2trash(str(path))
                    outcomes.append(
                        RepairOutcome(path=path, action="recycle shortcut", ok=True, detail="moved to Recycle Bin")
                    )
                else:
                    os.unlink(str(path))
                    outcomes.append(
                        RepairOutcome(path=path, action="delete shortcut", ok=True, detail="shortcut deleted")
                    )
                continue

            # -- Dangling symlinks / junctions -----------------------------
            if isinstance(item, BrokenSymlink) or link_type == "symlink" or link_type in ("junction", "mount_point"):
                ...
                if dry_run:
                    ...
                    outcomes.append(
                        RepairOutcome(path=path, action=action, ok=True, detail="planned: remove link only")
                    )
                    continue
                try:
                ...
                outcomes.append(RepairOutcome(path=path, action="remove symlink", ok=True, detail="link removed"))
                continue
            ...
            outcomes.append(RepairOutcome(path=path, action="failed", ok=False, detail=str(exc)))

    return outcomes


class BrokenLinkDetector:
    """Scans folders for dangling symlinks/junctions and broken .lnk shortcuts, scoring confidence."""
    ...
    def scan_symlinks(self, path: str) -> List[BrokenSymlink]:
        ...
        try:
            for root, dirs, files in os.walk(scan_path, followlinks=False):
                ...
                self._emit(
                    f"Scanning symlinks: {self.scan_count:,} items \u2014 "
    ...
    def scan_windows_shortcuts(self, path: str) -> List[BrokenShortcut]:
        ...
        try:
            ...
            for lnk_file in scan_path.rglob("*.lnk"):
                ...
                if checked % 25 == 0:
                    self._emit(
                        f"Scanning shortcuts: {checked} .lnk files \u2014 "
    ...
    def _get_search_locations(self, original_path: Path) -> List[Path]:
        ...
        if self.is_windows:
            ...
            program_dirs = [Path(program_files), Path(program_files_x86), Path(local_app_data), Path(app_data)]
    ...
    def attempt_repair(self, broken_link: BrokenLink) -> RepairResult:
        """Retarget one broken symlink/shortcut to its first moved-target candidate, with backup.
        ...
        if not potential_targets:
            return RepairResult(
                success=False, original_path=broken_link.path, error_message="No potential targets found"
            )
        ...
        try:
            if isinstance(broken_link, BrokenSymlink):
                ...
                return RepairResult(success=False, original_path=broken_link.path, error_message="Unknown link type")
    ...
    def scan_all(self, path: str, progress=None, cancel_event=None, include_registry: bool = False) -> List[BrokenLink]:
        ...
        self.logger.info(
            f"Scan complete. Found {len(self.broken_links)} broken links "
    ...
    def _cancelled(self) -> bool:
        """True when the scan cancel event was set.
    ...
    def _emit(self, text: str) -> None:
        """Invoke the progress callback with status text, never letting UI errors break the scan.
