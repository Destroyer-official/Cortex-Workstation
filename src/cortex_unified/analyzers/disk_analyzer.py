...
class DiskAnalyzer:
    ...
    def analyze_directory_tree(self, max_depth: int = 3) -> Dict:
        ...
        """
        self.directory_tree = self._analyze_directory_recursive(self.root_path, max_depth, 0)
                ...
                try:
                    for item in path.iterdir():
                        ...
                        if item.is_file():
                            ...
                            child_info = self._analyze_directory_recursive(item, max_depth, current_depth + 1)
                            if child_info:
    ...
    def analyze_file_types(self) -> Dict[str, Dict]:
                ...
                for file in files:
                    ...
                    try:
                        ...
                        if ext not in self.file_type_breakdown:
                            self.file_type_breakdown[ext] = {"count": 0, "size_bytes": 0}
        ...
        self.file_type_breakdown = dict(
            sorted(self.file_type_breakdown.items(), key=lambda x: x[1]["size_bytes"], reverse=True)
        )
    ...
    def get_stats(self) -> dict:
        ...
        largest_dirs_info = [
            {"path": str(path), "size_bytes": size, "size_human": self._format_bytes(size)}
