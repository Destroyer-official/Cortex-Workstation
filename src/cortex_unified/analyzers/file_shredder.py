...
class FileShredder:
    ...
    def shred_file(self, filepath: Path, passes: int = None, allow_system_files: bool = False) -> bool:
        ...
        try:
            ...
            if not is_safe:
                self.errors.append({"file": str(filepath), "error": f"Security check failed: {reason}"})
                return False
            ...
            if self.verify and filepath.exists():
                self.errors.append({"file": str(filepath), "error": "File still exists after deletion"})
                return False
    ...
    def shred_files(self, filepaths: List[Path], passes: int = None) -> Dict[str, int]:
        ...
        return {"shredded": shredded_count, "errors": error_count, "total": len(filepaths)}
    ...
    def get_stats(self) -> dict:
        ...
        """
        return {"files_shredded": len(self.shredded_files), "errors": len(self.errors), "passes_per_file": self.passes}
