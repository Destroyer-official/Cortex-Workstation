...
@dataclass
class DetectedItem:
    """Base record for one detected leftover item with confidence, size, and reasons."""
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Serialize this item to a dict with path and timestamp as strings.
...
class OrphanedFolder(DetectedItem):
    """Folder flagged as a possible app leftover, with install-location type and content flags."""
...
class RegistryOrphan(DetectedItem):
    """Registry key whose referenced file path no longer exists on disk."""
...
class LeftoverDetector:
    """Scans install roots, download/temp folders, and registry for orphaned leftovers."""

    def __init__(self, config: Config = None):
        """Store config, init result lists, stats, detection patterns, and install roots.
        ...
        # Statistics
        self.scan_stats = {"folders_scanned": 0, "files_scanned": 0, "registry_keys_scanned": 0, "errors": 0}
    ...
    def _setup_installation_paths(self):
        ...
        if sys.platform.startswith("win"):
            self.installation_paths = {
                ...
                "appdata": [Path(os.environ.get("APPDATA", "")), Path(os.environ.get("LOCALAPPDATA", ""))],
                "temp": [
    ...
    def _load_detection_patterns(self):
        ...
        self.orphaned_folder_patterns = {
            "uninstaller_remnants": [r".*uninstall.*", r".*uninst.*", r".*remove.*"],
            "temp_install_folders": [r".*_temp_.*", r".*\.tmp.*", r".*install.*temp.*", r".*setup.*temp.*"],
            "version_folders": [
                r".*\d+\.\d+.*",  # Version numbers
                r".*v\d+.*",  # Version prefixes
                r".*_\d{4}.*",  # Year patterns
            ],
            "backup_folders": [r".*backup.*", r".*\.bak.*", r".*_old.*", r".*\.old.*"],
        }
    ...
    def _folder_appears_abandoned(self, folder: Path) -> bool:
        ...
        try:
            ...
            non_temp_files = [item for item in items if item.is_file() and item.suffix.lower() not in temp_extensions]
    ...
    def _contains_config_files(self, folder: Path) -> bool:
        ...
        try:
            for item in folder.rglob("*"):
                if item.is_file():
                    if item.suffix.lower() in config_extensions or any(
                        name in item.name.lower() for name in config_names
                    ):
                        return True
        except Exception:
            pass
    ...
    def detect_installer_files(self, paths: List[str] = None) -> List[InstallerFile]:
        """Scan download/temp roots for installer files by platform extension.
        ...
        if paths is None:
            ...
            if sys.platform.startswith("win"):
                ...
                home = Path.home()
                scan_paths.extend([home / "Downloads", Path("/tmp"), Path("/var/tmp")])
        else:
            scan_paths = [normalize_path(p) for p in paths]
    ...
    def _extract_version_from_filename(self, filename: str) -> Optional[str]:
        ...
        version_patterns = [r"v?(\d+\.\d+\.\d+)", r"v?(\d+\.\d+)", r"(\d{4}\.\d+)", r"_v?(\d+\.\d+)"]
                    ...
                    try:
                        ...
                        if isinstance(value, str) and (value.startswith("C:") or value.startswith('"C:')):
                            ...
                            if not Path(clean_path).exists():
                                orphan = self._create_registry_orphan(full_key_path, hive_name, clean_path, "uninstall")
                                if orphan:
                                    with self._lock:
    ...
    def _create_registry_orphan(
        self, registry_key: str, hive: str, referenced_path: str, key_type: str
    ) -> Optional[RegistryOrphan]:
