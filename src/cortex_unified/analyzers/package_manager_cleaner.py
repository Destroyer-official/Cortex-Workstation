...
class Package:
    ...
    Manages Package operations and coordinates related state changes for the component.
    """
...
class PackageManager:
    ...
    Manages PackageManager operations and coordinates related state changes for the component.
    """
...
class HealthStatus:
    ...
    Manages HealthStatus operations and coordinates related state changes for the component.
    """
...
class PackageManagerCleaner:
    ...
    def _get_package_manager_version(self, name: str, executable: str) -> Optional[str]:
        ...
        Manages get package manager version operations and coordinates related state changes for the component.
    ...
    def _get_cache_path(self, name: str, config: Dict) -> Optional[Path]:
        ...
        Manages get cache path operations and coordinates related state changes for the component.

        Args:
            name (str): The name parameter.
            config (Dict): The config parameter.
        ...
        if override:
            ...
            if name == "scoop":
                ...
                try:
                    result = _proc.run([config["executable"]] + config.get("cache_commands", []), text=True, timeout=10)
        ...
        try:
            ...
            if name == "pip":
                result = _proc.run([executable] + cache_commands, text=True, timeout=10)
                if result.returncode == 0:
    ...
    def clean_npm_cache(self, verify_integrity: bool = True) -> CleanupResult:
        ...
        try:
            ...
            result = _proc.run([npm_manager.executable, "cache", "clean", "--force"], text=True, timeout=60)

            if result.returncode != 0:
                return CleanupResult(False, 0, 0, [f"NPM cache clean failed: {result.stderr}"])

            if verify_integrity:
                verify_result = _proc.run([npm_manager.executable, "cache", "verify"], text=True, timeout=60)
    ...
    def _find_pip_orphaned_packages(self, manager: PackageManager) -> List[Package]:
        ...
        """
        try:
            result = _proc.run([manager.executable, "list", "--format=json"], text=True, timeout=30)
            if result.returncode != 0:
                return []
    ...
    def _find_npm_orphaned_packages(self, manager: PackageManager) -> List[Package]:
        ...
        Manages find npm orphaned packages operations and coordinates related state changes for the component.

        Args:
            manager (PackageManager): The manager parameter.

        Returns:
            List[Package]: List of processed items or identifiers.
        """
        try:
            result = _proc.run([manager.executable, "ls", "--json", "--depth=0"], text=True, timeout=30)
    ...
    def _find_apt_orphaned_packages(self, manager: PackageManager) -> List[Package]:
        ...
        Manages find apt orphaned packages operations and coordinates related state changes for the component.
        ...
        """
        try:
            orphaned_result = _proc.run(["apt", "autoremove", "--dry-run"], text=True, timeout=30)
    ...
    def _find_dnf_orphaned_packages(self, manager: PackageManager) -> List[Package]:
        ...
        Manages find dnf orphaned packages operations and coordinates related state changes for the component.
        ...
        """
        try:
            result = _proc.run([manager.executable, "repoquery", "--unneeded"], text=True, timeout=30)
            if result.returncode != 0:
                result = _proc.run([manager.executable, "leaves"], text=True, timeout=30)
            if result.returncode != 0:
                return []
    ...
    def _find_pacman_orphaned_packages(self, manager: PackageManager) -> List[Package]:
        ...
        """
        try:
            result = _proc.run([manager.executable, "-Qtdq"], text=True, timeout=30)
            if result.returncode != 0:
                return []
            ...
            for name in orphaned_names:
                if name.strip():
                    package = Package(name=name.strip(), version="unknown", size=0, manager="pacman", is_orphaned=True)
                    packages.append(package)
    ...
    def _find_brew_orphaned_packages(self, manager: PackageManager) -> List[Package]:
        ...
        Manages find brew orphaned packages operations and coordinates related state changes for the component.
    ...
    def verify_package_manager_health(self, package_manager: str) -> HealthStatus:
        ...
        try:
            ...
            if package_manager == "npm":
                doctor_result = _proc.run([manager.executable, "doctor"], text=True, timeout=60)
                if doctor_result.returncode != 0:
                    issues.append("npm doctor reported issues")
                    recommendations.append("Run 'npm doctor' for detailed diagnostics")

            elif package_manager == "pip":
                check_result = _proc.run([manager.executable, "check"], text=True, timeout=30)
    ...
    def _get_manager_by_name(self, name: str) -> Optional[PackageManager]:
        ...
        Manages get manager by name operations and coordinates related state changes for the component.

        Args:
            name (str): The name parameter.

        Returns:
            Optional[PackageManager]: Result of the operation.
        """
    ...
    def _get_cache_size(self, cache_path: Optional[Path]) -> int:
        ...
        Manages get cache size operations and coordinates related state changes for the component.

        Args:
            cache_path (Optional[Path]): Filesystem path to the target file or directory.
    ...
    def get_stats(self) -> Dict[str, any]:
        ...
        """
        stats = {"detected_managers": len(self.detected_managers), "managers": {}}
    ...
    def _scan_project_caches_in_folder(
        self,
        folder: Path,
        ...
        def _match_dir(d_name: str) -> Optional[tuple]:
            ...
            Returns:
                Optional[tuple]: Result of the operation.
            """
    ...
    def _scan_manager_cache(self, manager: PackageManager, keep_recent_days: int) -> Optional[Dict]:
        ...
        try:
            total_size, file_count = self._get_dir_size(
                manager.cache_path, datetime.now() - timedelta(days=keep_recent_days)
            )
    ...
    def auto_discover_project_caches(
        ...
        try:
            ...
            drive_roots = [
                ...
                or p.fstype.lower() in ("ntfs", "refs")
                or (platform.system() == "Windows" and len(p.mountpoint) == 3 and p.mountpoint[1] == ":")
            ]
        ...
        for idx, resource in enumerate(resources):
            ...
            try:
                ...
                if resource_type in ("python_cache", "project_cache"):
                    ...
                    result = self._cleanup_manager_cache(resource.get("manager_name"), dry_run)
                else:
                    continue
    ...
    def _cleanup_python_cache(self, cache_path: Path, dry_run: bool = True) -> Dict:
        ...
        return {"freed": freed, "removed": removed, "errors": errors, "dry_run": dry_run}
    ...
    def _cleanup_manager_cache(self, manager_name: str, dry_run: bool = True) -> Dict:
        ...
        manager = self._get_manager_by_name(manager_name)
        if not manager:
            return {"freed": 0, "removed": 0, "errors": [f"Package manager {manager_name} not found"]}
        ...
        try:
            ...
            if result.returncode != 0:
                return {"freed": 0, "removed": 0, "errors": [f"Clean command failed: {result.stderr}"]}

            cache_size_after = self._get_cache_size(manager.cache_path)
            freed = cache_size_before - cache_size_after

            return {"freed": freed, "removed": 1, "errors": [], "backup": backup_result.get(manager_name)}

        except Exception as e:
            return {"freed": 0, "removed": 0, "errors": [f"Cleanup failed: {e}"]}
