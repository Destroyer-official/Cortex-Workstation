...
@main.command()
@click.option(
    "--dry-run", is_flag=True, default=None, help="Show what would be deleted without actually deleting (default)"
)
...
@click.option(
    "--pattern", multiple=True, help="Only consider files matching this glob pattern (can be used multiple times)"
)
@click.option("--older-than", type=int, default=None, help="Only consider files older than N days")
@click.option(
    "--exclude-pattern",
    multiple=True,
    help="Exclude files/directories matching this pattern (can be used multiple times)",
)
...
@click.option(
    "--cpu-priority", type=click.Choice(["low", "normal", "high"]), default="normal", help="CPU priority for scanning"
)
@click.option(
    "--io-priority", type=click.Choice(["low", "normal", "high"]), default="low", help="I/O priority for scanning"
)
...
def clean_empty(
    ...
    try:
        ...
        if not dry_run_mode:
            if not yes:
                click.confirm(
                    f"Delete {stats['total_empty_count']} empty items? This action cannot be undone.", abort=True
                )
            else:
...
def find_large_files(
    min_size, pattern, exclude_pattern, config, no_config, verbose, log_file, json_log, threads, export, path
):
...
@click.option(
    "--strategy",
    type=click.Choice(["keep_newest", "keep_oldest", "keep_largest", "keep_smallest"]),
    default="keep_newest",
    help="Strategy for auto-selecting duplicates",
)
@click.option(
    "--hash-algorithm",
...
def find_duplicates(
    ...
    try:
        ...
        if delete and duplicates:
            ...
            if not yes:
                click.confirm(
                    f"Delete {len(files_to_delete)} duplicate files? This action cannot be undone.", abort=True
                )
            else:
                logger.info("Skipping confirmation due to --yes flag")
...
@main.command()
@click.option(
    "--dry-run", is_flag=True, default=True, help="Show what would be deleted without actually deleting (default)"
)
...
@click.option("--log-file", type=click.Path(), help="Write logs to file")
@click.option("--json-log", is_flag=True, default=False, help="Output logs in JSON format")
def clean_temp(dry_run, delete, trash, min_age, exclude_pattern, config, no_config, yes, verbose, log_file, json_log):
...
@click.option(
    "--export-treemap",
    type=click.Path(),
    help="Export TreeMap visualization (HTML/PNG/SVG) - hierarchical disk usage map",
)
@click.option(
    "--export-sunburst",
    type=click.Path(),
    help="Export Sunburst visualization (HTML/PNG/SVG) - circular directory tree",
)
@click.option(
    "--export-dashboard",
    type=click.Path(),
    help="Export Interactive Dashboard (HTML/PNG/SVG) - comprehensive analysis view",
)
@click.option(
    "--max-depth", type=int, default=3, help="Maximum directory depth for visualization analysis (deeper = more detail)"
)
@click.option("--threads", type=int, default=0, help="Number of threads to use for scanning (default: CPU count)")
@click.option(
    "--cpu-priority",
    ...
    default="normal",
    help="CPU priority for scanning process",
)
@click.option(
    "--io-priority",
    type=click.Choice(["low", "normal", "high"]),
    default="low",
    help="I/O priority for disk operations",
)
@click.option("--memory-limit", type=int, default=0, help="Memory limit in MB (0 = no limit, prevents system overload)")
@click.option(
    "--checkpoint-interval", type=int, default=1000, help="Save checkpoint every N directories (for resumable scans)"
)
...
def analyze_disk(
    ...
    try:
        ...
        logger.info(
            f"Disk usage: {disk_usage['used_human']} used of {disk_usage['total_human']} ({disk_usage['used_percent']:.1f}%)"
        )
...
def analyze_processes(export):
    ...
    try:
        ...
        if export:
            ...
            export_data = {"processes": processes, "services": services, "stats": stats}
            with open(export, "w", encoding="utf-8") as f:
                json.dump(export_data, f, indent=2)
            logger.info(f"Results exported to: {export}")
...
@main.command()
@click.option(
    "--dry-run", is_flag=True, default=True, help="Show what would be cleaned without actually cleaning (default)"
)
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Clean all Docker resources (images, containers, volumes, networks)",
)
@click.option("--config", type=click.Path(exists=False), help="Path to configuration file")
@click.option("--no-config", is_flag=True, default=False, help="Don't load any configuration file")
...
def docker_cleanup(
    ...
    try:
        ...
        if not dry_run:
            if not yes:
                click.confirm(
                    f"Clean {len(resources_to_clean)} Docker resources? This action cannot be undone.", abort=True
                )
            else:
                logger.info("Skipping confirmation due to --yes flag")
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Clean system package manager cache (apt, dnf, pacman, brew, chocolatey)",
)
@click.option(
    "--all", "clean_all", is_flag=True, default=False, help="Clean all detected package managers automatically"
)
@click.option(
    "--orphaned", is_flag=True, default=False, help="Find and clean orphaned packages (packages no longer needed)"
)
@click.option(
    "--keep-recent-days", type=int, default=7, help="Keep cache files newer than N days (preserves recent downloads)"
)
@click.option(
    "--dry-run", is_flag=True, default=True, help="Show what would be cleaned without actually cleaning (default)"
)
...
def package_cleanup(
    ...
    try:
        ...
        for manager in available_managers:
            should_clean = (
                clean_all
                or (pip and manager.name == "pip")
                or (npm and manager.name == "npm")
                or (yarn and manager.name == "yarn")
                or (conda and manager.name == "conda")
                or (system and manager.name in ["apt", "dnf", "pacman", "brew", "chocolatey"])
            )
...
@click.option(
    ...
    type=float,
    default=0.7,
    help="Minimum confidence score for leftover detection (0.0-1.0, higher = more certain)",
)
@click.option(
    "--scan-registry",
    is_flag=True,
    default=False,
    help="Include Windows registry analysis for orphaned entries (Windows only)",
)
@click.option(
    "--ml-patterns", is_flag=True, default=True, help="Use machine learning patterns for intelligent leftover detection"
)
@click.option(
    "--dry-run", is_flag=True, default=True, help="Show what would be cleaned without actually cleaning (default)"
)
...
@click.option(
    "--verbose", is_flag=True, default=False, help="Enable verbose output with confidence scores and reasoning"
)
@click.option("--log-file", type=click.Path(), help="Write logs to file")
@click.option("--json-log", is_flag=True, default=False, help="Output logs in JSON format")
...
def heuristics_scan(
    ...
    try:
        ...
        high_confidence_items = [
            item for item in all_items if detector.calculate_confidence_score(item) >= confidence_threshold
        ]
        ...
        if not dry_run and high_confidence_items:
            if not yes:
                click.confirm(
                    f"Clean {len(high_confidence_items)} high-confidence leftovers? This action cannot be undone.",
                    abort=True,
                )
            else:
                logger.info("Skipping confirmation due to --yes flag")
...
def secure_delete(shred, passes, verify, yes, verbose, log_file, json_log, files):
    ...
    try:
        ...
        if shred:
            if not yes:
                click.confirm(
                    f"Securely delete {len(files)} files with {passes} passes? This action cannot be undone.",
                    abort=True,
                )
            else:
                logger.info("Skipping confirmation due to --yes flag")
...
@click.option("--log-file", type=click.Path(), help="Write logs to file")
@click.option("--json-log", is_flag=True, default=False, help="Output logs in JSON format")
def restore(restore, dry_run, yes, verbose, log_file, json_log):
...
@click.option("--log-file", type=click.Path(), help="Write logs to file")
@click.option("--json-log", is_flag=True, default=False, help="Output logs in JSON format")
def generate_report(type, export, name, verbose, log_file, json_log):
    ...
    try:
        ...
        data = {
            ...
            "summary": {"reports_generated": len(generator.list_reports()) + 1, "report_type": type},
        }
...
def scan_enhanced(
    ...
    scanner = Scanner(
        config_obj, str(target_path), enable_checkpoints=enable_checkpoints, enable_throttling=enable_throttling
    )
...
def scan_broken_links(
    scan_symlinks, scan_shortcuts, scan_registry, repair, backup, confidence_threshold, export, verbose, path
):
...
@click.option(
    "--min-age-days", default=0, type=int, help="Minimum age in days to consider a shader file stale (default: 0)"
)
...
def clean_ai_cmd(dry_run: bool):
    ...
    if not dry_run:
        ...
        click.echo(
            f"Cleaned {res.cleaned_items} cache files, checkpointed {res.truncated_wal_count} WAL journals, freed {format_bytes(res.freed_bytes)}."
        )
