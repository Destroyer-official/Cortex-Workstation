...
class Deleter:
    ...
    def _delete_file(self, filepath: Path) -> bool:
        ...
        try:
            ...
            if not self.dry_run:
                ...
                if not is_safe:
                    self.errors.append(
                        {"type": "file", "path": str(filepath), "error": f"Security check failed: {reason}"}
                    )
                    return False
            ...
            if self.dry_run:
                self.deleted_items.append({"type": "file", "path": str(filepath), "action": "would_delete"})
                return True

            if self.use_trash:
                send2trash(str(filepath))
                self.deleted_items.append({"type": "file", "path": str(filepath), "action": "moved_to_trash"})
            else:
                filepath.unlink()
                self.deleted_items.append({"type": "file", "path": str(filepath), "action": "deleted"})
            return True
        except Exception as e:
            # One unreadable/locked path must not abort a batch run, so
            # failures are recorded instead of raised.
            self.errors.append({"type": "file", "path": str(filepath), "error": str(e)})
            return False
    ...
    def _delete_directory(self, dirpath: Path) -> bool:
        ...
        try:
            ...
            if self.dry_run:
                self.deleted_items.append({"type": "directory", "path": str(dirpath), "action": "would_delete"})
                return True

            if self.use_trash:
                send2trash(str(dirpath))
                self.deleted_items.append({"type": "directory", "path": str(dirpath), "action": "moved_to_trash"})
            else:
                dirpath.rmdir()
                self.deleted_items.append({"type": "directory", "path": str(dirpath), "action": "deleted"})
            return True
        except Exception as e:
            # Same rationale as _delete_file: record and continue.
            self.errors.append({"type": "directory", "path": str(dirpath), "error": str(e)})
            return False
