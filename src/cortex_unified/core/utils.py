...
def get_component_logger(
    component: str, verbose: bool = False, log_file: str = None, json_logging: bool = False
) -> logging.Logger:
...
def log_operation_start(logger: logging.Logger, operation: str, **kwargs) -> dict:
    ...
    """
    context = {"operation": operation, "start_time": datetime.now(), **kwargs}
...
def log_operation_end(
    logger: logging.Logger, context: dict, success: bool = True, error: Exception = None, **kwargs
) -> None:
    ...
    result_context = {**context, "duration": duration, "success": success, **kwargs}

    if success:
        logger.info(f"Completed {context['operation']} in {duration:.1f}ms", extra=result_context)
    else:
        logger.error(
            f"Failed {context['operation']} after {duration:.1f}ms: {error}", extra=result_context, exc_info=error
        )
...
def log_performance_metrics(logger: logging.Logger, operation: str, metrics: dict) -> None:
    ...
    """
    context = {"operation": operation, "metrics": True, **metrics}
...
class DeepCleanerError(Exception):
    ...
    def __init__(
        self, message: str, operation: str = None, component: str = None, error_code: str = None, details: dict = None
    ):
...
def handle_error(
    logger: logging.Logger, error: Exception, operation: str = None, component: str = None, reraise: bool = True
) -> None:
    ...
    if isinstance(error, DeepCleanerError):
        error_context.update({"error_code": error.error_code, "details": error.details})

    logger.error(f"Error in {operation or 'unknown operation'}: {error}", extra=error_context, exc_info=True)

    if reraise:
        raise error


def safe_execute(func, logger: logging.Logger, operation: str = None, default_return=None, **kwargs):
...
def get_nexus_native_dir() -> Path | None:
    ...
    if getattr(sys, "frozen", False):
        ...
        if meipass:
            ...
            candidates.append(Path(meipass) / "native")
