...
class MultiUserScanner:
    ...
    def _detect_windows_user_profiles_enhanced(self) -> List[UserProfile]:
        ...
        try:
            ...
            users_dir = (
                ...
                else Path(os.environ.get("SystemDrive", "C:") + "/Users")
            )
            if users_dir.exists():
    ...
    def _get_unix_last_login(self, username: str) -> Optional[datetime]:
        ...
        try:
            ...
            result = subprocess.run(["last", "-1", username], capture_output=True, text=True, timeout=10)
    ...
    def scan_user_profile(self, profile: UserProfile, scanner_factory: Optional[Callable] = None) -> Dict[str, Any]:
        ...
        if not profile.is_accessible:
            return {"error": "Access denied", "empty_files": [], "empty_dirs": [], "profile_info": asdict(profile)}
        ...
        try:
            ...
            return {"error": str(e), "empty_files": [], "empty_dirs": [], "profile_info": asdict(profile)}
    ...
    def _store_credentials(self, credentials: Dict[str, str]) -> None:
        ...
        for server, cred_info in credentials.items():
            try:
                ...
                self._network_credentials[server] = {"username": username, "password": password}
...
class MultiDriveScanner:
    ...
    def scan_multiple_drives(
        self, drives: List[str], parallel: bool = True, scanner_factory: Optional[Callable] = None
    ) -> Dict[str, Any]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_drive):
                ...
                try:
                    ...
                    results[drive] = {"error": str(e), "empty_files": [], "empty_dirs": [], "location_type": "drive"}
    ...
    def _scan_single_drive_with_progress(self, drive_path: str, scanner_factory: Callable) -> Dict[str, Any]:
        ...
        try:
            if not os.path.exists(drive_path):
                return {"error": "Drive not accessible", "empty_files": [], "empty_dirs": [], "location_type": "drive"}
            ...
            try:
                ...
                drive_info = {"total_size": usage.total, "free_size": usage.free, "used_size": usage.used}
            except Exception:
                pass
    ...
    def scan_multiple_locations(self, locations: List[Dict[str, Any]], parallel: bool = True) -> Dict[str, Any]:
            ...
            for i, location in enumerate(locations):
                ...
                try:
                    if location_type == "drive":
                        result = self._scan_single_drive_with_progress(location_path, location.get("scanner_factory"))
