"""Performance optimization utilities for Cortex Cleaner operations."""
...
class PerformanceOptimizer:
    ...
    def _optimize_garbage_collection(self) -> None:
        ...
        new_thresholds = tuple(
            int(threshold * self.settings.gc_threshold_multiplier) for threshold in self.original_gc_thresholds
        )
        ...
        def monitor_memory():
            ...
            while self.memory_monitor_active:
                try:
                    ...
                    # Check memory limit
                    if self.settings.max_memory_mb > 0 and memory_mb > self.settings.max_memory_mb:
                        self.logger.warning(
                            f"Memory usage ({memory_mb:.1f} MB) exceeds limit ({self.settings.max_memory_mb} MB)"
                        )
    ...
    def optimize_for_operation(self, operation_name: str, operation_func: Callable, *args, **kwargs) -> Any:
        ...
        try:
            ...
            self.logger.debug(
                f"Operation {operation_name} completed in {operation_time:.3f}s, " f"memory delta: {memory_used:.1f}MB"
            )
