"""Resource monitoring and management for Cortex Cleaner operations."""
...
class ResourceMonitor:
    ...
    def get_metrics_summary(self, duration_minutes: int = 5) -> Dict:
        ...
        return {
            ...
            "cpu": {"avg": sum(cpu_values) / len(cpu_values), "min": min(cpu_values), "max": max(cpu_values)},
            "memory": {
    ...
    def should_throttle_operations(self) -> bool:
        ...
        return (
            current_metrics.cpu_percent > self.cpu_warning_threshold
            or current_metrics.memory_percent > self.memory_warning_threshold
            or current_metrics.memory_available_mb < 200
        )
