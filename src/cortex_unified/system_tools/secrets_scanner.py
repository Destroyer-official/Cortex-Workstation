...
class Finding:
    ...
    def fingerprint(self) -> str:
        ...
        return hashlib.sha256(f"{self.file_path}|{self.pattern_name}|{self.match_preview}".encode()).hexdigest()[:16]
...
PATTERNS: List[DetectionPattern] = [
    ...
    DetectionPattern(
        ...
        regex=_p(
            r"(?:client_secret|clientSecret|AccountKey|AZURE_CLIENT_SECRET)\s*[=:]\s*['\"]?([A-Za-z0-9+/=_\-]{32,})['\"]?"
        ),
        severity="CRITICAL",
        category="SECRET",
        compliance=["SOC2", "PCI_DSS"],
    ...
    DetectionPattern(
        name="npm Auth Token",
        regex=_p(
            r"(?://registry\.npmjs\.org/:_authToken|npm_[A-Za-z0-9]{36,}|NPM_TOKEN)\s*[=:]\s*['\"]?([A-Za-z0-9_\-]{32,})['\"]?"
        ),
        severity="HIGH",
        category="SECRET",
        compliance=["SOC2"],
    ...
    DetectionPattern(
        ...
        regex=_p(
            r"(?:DATABASE_URL|DB_URL|database_url)\s*[=:]\s*['\"]?((?:postgres(?:ql)?|mysql|mongodb(?:\+srv)?|redis|mssql)://[^:@\s'\"]{1,60}:[^@\s'\"]{1,100}@[^\s'\"]{3,})['\"]?",
            re.MULTILINE,
        ),
        severity="CRITICAL",
        category="SECRET",
    ...
    DetectionPattern(
        ...
        regex=_p(
            r"(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|3[47][0-9]{13}|3(?:0[0-5]|[68][0-9])[0-9]{11}|6(?:011|5[0-9]{2})[0-9]{12})"
        ),
        severity="CRITICAL",
        category="PII",
    ...
    DetectionPattern(
        ...
        regex=_p(
            r"""(?:execute|cursor\.execute|db\.query)\s*\([^)]*(?:\+|\%\s+|\.format\(|f["'])[^)]*(?:SELECT|INSERT|UPDATE|DELETE|DROP)"""
        ),
        severity="HIGH",
        category="CONFIG",
        compliance=["SOC2", "PCI_DSS"],
...
TEST_PATH_RE = re.compile(
    r"(?:test|spec|fixture|mock|example|sample|demo|fake|stub|dummy|__test__|\.test\.|\.spec\.|_test\.|_spec\.)",
    re.IGNORECASE,
)
PLACEHOLDER_RE = re.compile(
    r"^(?:your[_\-]?(?:api[_\-]?key|secret|token|password)|insert[_\-]?(?:key|token)|xxx+|abc+123*|password123|changeme|replace[_\-]?me|example[_\-]?(?:key|token|secret)|<[^>]+>|\$\{[^}]+\}|\{\{[^}]+\}\}|XXXXXXXX+)$",
    re.IGNORECASE,
)
...
def scan_git_history(directory: str, max_commits: int = 500, quiet: bool = False) -> Tuple[List[Finding], int]:
    ...
    try:
        ...
        for commit_hash in commits:
            diff = subprocess.run(
                ["git", "-C", directory, "show", "--unified=0", commit_hash], capture_output=True, timeout=30
            )
...
def _vr(
    finding_id: str, name: str, live: Optional[bool], identity: Optional[str], blast: str, err: Optional[str] = None
) -> VerificationResult:
    ...
    return VerificationResult(
        finding_id=finding_id, pattern_name=name, is_live=live, identity=identity, blast_radius=blast, error=err
    )
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as r:
            ...
            return _vr(
                ...
                True,
                arn,
                f"🔴 LIVE AWS key. Identity: {arn}. Check CloudTrail immediately. " "Rotate in AWS IAM Console NOW.",
            )
    except urllib.error.HTTPError as e:
...
def verify_github(token: str) -> VerificationResult:
    ...
    status, data = _http(
        "https://api.github.com/user", {"Authorization": f"token {token}", "User-Agent": "Sentinel/2.0"}
    )
    if status == 200:
...
def verify_stripe(key: str) -> VerificationResult:
    ...
    status, data = _http(
        "https://api.stripe.com/v1/balance", {"Authorization": f"Basic {auth}", "User-Agent": "Sentinel/2.0"}
    )
    if status == 200:
...
def verify_slack(token: str) -> VerificationResult:
    ...
    status, data = _http(
        "https://slack.com/api/auth.test", {"Authorization": f"Bearer {token}", "User-Agent": "Sentinel/2.0"}
    )
...
def verify_npm(token: str) -> VerificationResult:
    ...
    status, data = _http(
        "https://registry.npmjs.org/-/whoami", {"Authorization": f"Bearer {token}", "User-Agent": "Sentinel/2.0"}
    )
    if status == 200:
...
def verify_openai(key: str) -> VerificationResult:
    ...
    status, data = _http(
        "https://api.openai.com/v1/models", {"Authorization": f"Bearer {key}", "User-Agent": "Sentinel/2.0"}
    )
    if status == 200:
        ...
        return _vr(
            ...
            "OpenAI Key",
            True,
            f"OpenAI account (models: {', '.join(models)})",
...
def verify_all_findings(findings: List[Finding], quiet: bool = False) -> Dict[str, VerificationResult]:
    ...
    for f in verifiable:
        ...
        try:
            ...
            if not quiet:
                print(
                    f"  {result.status_emoji} [{f.pattern_name}] in ...{f.file_path[-40:]}:{f.line_number}",
                    file=sys.stderr,
                )
                if result.identity:
        ...
        json.dump(
            {
                ...
                "fingerprints": {
                    f.fingerprint: {**f.to_dict(), "match_preview": _truncate_secret(f.match_preview)} for f in findings
                },
            },
            f,
            indent=2,
...
def create_jira_ticket(
    finding: Finding, jira_url: str, jira_user: str, jira_token: str, project_key: str
) -> Optional[str]:
    ...
    payload = json.dumps(
        {
            "fields": {
                ...
                "issuetype": {"name": "Bug"},
                "priority": {
                    "name": {"CRITICAL": "Highest", "HIGH": "High", "MEDIUM": "Medium", "LOW": "Low"}.get(
...
def export_sarif(stats: ScanStats, path: str):
    ...
    sarif = {
        ...
        "runs": [
            {
                "tool": {"driver": {"name": "Sentinel", "version": VERSION, "rules": list(rules.values())}},
                "results": results,
            }
        ],
    }
...
def generate_html_report(stats: ScanStats, output_path: str):
    ...
    for fw in ["GDPR", "HIPAA", "PCI_DSS", "SOC2"]:
        comp_buttons_list.append(f'<button class="filter-btn" onclick="filterCompliance(\'{fw}\')">{fw}</button>')
    compliance_section = (
...
"""
    if not stats.findings:
        ...
        for i, f in enumerate(stats.findings):
            ...
            blast_html = (
                f'<div class="blast-block"><div class="blast-title">⚡ Blast Radius</div><div style="font-size:13px;color:var(--text)">{_e(f.blast_radius or "")}</div></div>'
                if f.blast_radius
                else ""
            )
            identity_html = (
                f'<div class="detail-block"><div class="detail-label">Verified Identity</div><div class="detail-value" style="color:var(--accent)">{_e(f.identity or "")}</div></div>'
                if f.identity
                else ""
            )
            html += f"""
...
def print_terminal_report(stats: ScanStats):
    ...
    if RICH_AVAILABLE:
        ...
        console.print()
        console.print(
            f"  🔴 CRITICAL: {len(stats.critical)}  🟠 HIGH: {len(stats.high)}  🟡 MEDIUM: {len(stats.medium)}  🟢 LOW: {len(stats.low)}"
        )
        if stats.suppressed_count:
            console.print(f"  [dim]({stats.suppressed_count} findings suppressed as false positives)[/dim]")
        if stats.is_delta:
            console.print(
                f"  [cyan](Delta mode: showing {len(stats.findings)} NEW findings, {stats.delta_resolved} resolved)[/cyan]"
            )
        console.print()
    else:
    ...
    if getattr(args, "diff", False):
        ...
        if baseline is None:
            ...
            if not getattr(args, "quiet", False):
                print(
                    f"📊 Delta mode: {len(new_findings)} NEW findings, {resolved} resolved since baseline",
                    file=sys.stderr,
                )
...
def cmd_patterns(args):
    ...
    if RICH_AVAILABLE:
        ...
        for p in sorted(PATTERNS, key=lambda x: -SEVERITY_ORDER.get(x.severity, 0)):
            table.add_row(
                p.name, Text(p.severity, style=SEV_STYLE.get(p.severity, "white")), p.category, ", ".join(p.compliance)
            )
        console.print(table)
    else:
        for p in sorted(PATTERNS, key=lambda x: -SEVERITY_ORDER.get(x.severity, 0)):
...
def build_parser():
    ...
    sp.add_argument(
        "--github-repo", metavar="OWNER/REPO", help="GitHub repo for auto-issues (needs GITHUB_TOKEN env var)"
    )
    ...
    bsave = bsub.add_parser("save")
    bsave.add_argument("directory", nargs="?", default=".")
    bdiff = bsub.add_parser("diff")
    bdiff.add_argument("directory", nargs="?", default=".")
    bclear = bsub.add_parser("clear")
    bclear.add_argument("directory", nargs="?", default=".")

    # fp
    fp = sub.add_parser("fp", help="Manage false positive suppressions")
    fpsub = fp.add_subparsers(dest="fp_action")
    fpadd = fpsub.add_parser("add")
    fpadd.add_argument("fingerprint")
    fpadd.add_argument("--reason", default="")
    fplist = fpsub.add_parser("list")
    fprem = fpsub.add_parser("remove")
    fprem.add_argument("fingerprint")
