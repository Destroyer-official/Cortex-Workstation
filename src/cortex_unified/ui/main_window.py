...
class ScanWorker(QObject):
    ...
    def __init__(
        ...
        """Store scan config, target path, checkpoint/throttle flags, and the cancel flag.
        ...
        Args:
            ...
            enable_checkpoints (bool): The enable checkpoints parameter.
            enable_throttling (bool): The enable throttling parameter.
    ...
    def run(self):
        """Run the scanner (emits finished with (files, dirs), or error); relays checkpoint progress from a poller thread.
        ...
        try:
            ...
            if self.enable_checkpoints:
                ...
                def progress_monitor():
                    """Poll scanner progress every 0.5s and relay it via progress_updated until stopped.
...
class DeleteWorker(QObject):
    ...
    def __init__(self, deleter: Deleter, empty_files: List[Path], empty_dirs: List[Path]):
        """Store the deleter and the file/dir lists to remove.

        Initializes the instance and configures internal state.

        Args:
            deleter (Deleter): The deleter parameter.
            empty_files (List[Path]): The empty files parameter.
            empty_dirs (List[Path]): The empty dirs parameter.
        """
        super().__init__()
    ...
    def run(self):
        """Run the deletion (emits finished with the result dict, or error).
...
class MultiDriveScanWorker(QObject):
    ...
    def __init__(
        self, config: Config, paths: List[str], enable_checkpoints: bool = False, enable_throttling: bool = False
    ):
        """Store config, drive paths, checkpoint/throttle flags, and pause/stop flags.
    ...
    def run(self):
        ...
        try:
            ...
            scanner = MultiDriveScanner(
                self.config, enable_checkpoints=self.enable_checkpoints, enable_throttling=self.enable_throttling
            )
            if self.enable_checkpoints:
                import threading
                import time
                ...
                def progress_monitor():
                    """Poll overall multi-drive progress every 0.5s unless paused/stopped.
...
class DeepCleanerGUI(QMainWindow):
        ...
        try:
            ...
            if len(valid_paths) > 1:
                self.scan_worker = MultiDriveScanWorker(
                    scan_config, valid_paths, enable_checkpoints=enable_checkpoints, enable_throttling=enable_throttling
                )
            else:
    ...
    def scan_error(self, error: str):
        """Reset the scan controls and report the error to the user.
        ...
        if hasattr(progress, "percentage"):
            ...
            if hasattr(progress, "processed_count") and hasattr(progress, "total_count"):
                self.scan_stats_label.setText(
                    f"Processed: {progress.processed_count}/{progress.total_count} items ({progress.percentage:.1f}%)"
                )
    ...
    def start_delete(self):
        ...
        self.delete_worker = DeleteWorker(
            Deleter(dry_run=dry_run, use_trash=use_trash), self.empty_files, self.empty_dirs
        )
    ...
    def delete_finished(self, result: Dict[str, Any]):
        ...
        if hasattr(self, "logger"):
            ...
            self.logger.info(
                "Results - Files deleted: {}, Directories deleted: {}".format(
                    result["files_deleted"], result["dirs_deleted"]
                )
            )
    ...
    def refresh_processes(self):
        ...
        try:
            ...
            self.add_activity(
                f"Refreshed processes: {stats['total_processes']} processes, {stats['total_services']} services"
            )
        except Exception as e:
    ...
    def scan_registry(self):
        """Scan for orphaned registry entries and show them; enables the clean button.
    ...
    def clean_registry(self):
        """Back up the registry, then remove the scanned orphaned entries after confirmation.
        ...
        try:
            ...
            self.registry_results.setPlainText(
                "Registry cleaning completed. Run scan again to check for remaining issues."
            )
    ...
    def format_bytes(self, bytes_value: Union[int, float]) -> str:
        ...
        Args:
            bytes_value (Union[int, float]): The bytes value parameter.
    ...
    def repair_selected_links(self):
        ...
        repairable_links = [
            link for link in selected_links if link.is_repairable and link.confidence_score >= confidence_threshold
        ]
        if not repairable_links:
            QMessageBox.information(
                self, "No Repairable Links", "None of the selected links meet the confidence threshold for repair."
            )
            return
        reply = QMessageBox.question(
            self,
            "Confirm Repair",
            f"Repair {len(repairable_links)} broken links?\n\nBackups will be created: {('Yes' if self.create_backups_checkbox.isChecked() else 'No')}",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
        )
        if reply != QMessageBox.StandardButton.Yes:
            return
    ...
    def add_network_drive(self):
        ...
        test_button.clicked.connect(
            lambda: self.test_network_connection(
    ...
    def add_network_path(self, dialog, path, username, password):
        ...
        try:
            ...
            item = QListWidgetItem(drive_info)
            item.setData(
                Qt.ItemDataRole.UserRole, {"path": path, "username": username, "password": password, "type": "network"}
            )
            self.drives_list.addItem(item)
    ...
    def delete_checkpoint(self):
        ...
        try:
            ...
            if success:
                QMessageBox.information(
                    self, "Checkpoint Deleted", f"Checkpoint '{checkpoint_id}' deleted successfully."
                )
    ...
    def cleanup_old_checkpoints(self):
        """Prompt for an age and delete checkpoints older than that many days.
        ...
        try:
            ...
            age_days, ok = QInputDialog.getInt(
                self, "Cleanup Checkpoints", "Delete checkpoints older than how many days?", 7, 1, 365
            )
            if not ok:
                return
            scan_manager = ScanManager()
            ...
            QMessageBox.information(
                self, "Cleanup Complete", f"Deleted {deleted_count} checkpoints older than {age_days} days."
            )
    ...
    def create_file_shredder_tab(self) -> QWidget:
        ...
        warning_label.setStyleSheet(
            "QLabel { color: red; font-weight: bold; font-size: 14px; padding: 10px; background-color: #ffe6e6; border: 1px solid red; }"
        )
        ...
        self.start_shred_button.setStyleSheet(
            "QPushButton { font-weight: bold; padding: 5px 20px; background-color: #d32f2f; color: white; }"
        )
    ...
    def add_files_to_shred(self):
        ...
        for file_path in files:
            if file_path not in [
                self.shredder_file_list.item(i).text() for i in range(self.shredder_file_list.count())
            ]:
    ...
    def run_selected_task(self):
        ...
        try:
            ...
            if result.get("success", False):
                QMessageBox.information(
                    self, "Task Executed", f"Task '{task_name}' executed successfully.\n\n{result.get('message', '')}"
                )
    ...
    def generate_report(self):
        ...
        try:
            ...
            self.reports_progress_bar.setVisible(False)
            QMessageBox.information(
                self, "Report Generated", f"Report generated successfully!\n\nSaved to: {report_path}"
            )
    ...
    def schedule_report(self):
        ...
        try:
            ...
            schedule_type, ok = QInputDialog.getItem(
                self, "Schedule Report", "Select schedule frequency:", ["Daily", "Weekly", "Monthly"], 0, False
            )
    ...
    def create_auto_clean_rule(self):
        ...
        try:
            ...
            QMessageBox.information(
                self, "Rule Created", f"Auto-clean rule '{rule_name}' created successfully with ID: {rule_id}"
            )
    ...
    def test_selected_rule(self):
        ...
        try:
            ...
            if result.get("success", False):
                QMessageBox.information(
                    self, "Rule Test", f"Rule '{rule_name}' test completed successfully.\n\n{result.get('message', '')}"
                )
