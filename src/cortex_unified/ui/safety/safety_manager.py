...
class SafetyManager:
    ...
    def configure_safety_settings(
        ...
        self.logger.info(
            f"Updated safety settings: confirmation={self.require_confirmation}, "
    ...
    def create_operation(
        self, operation_type: OperationType, paths: List[Union[str, Path]], description: str = "", **parameters
    ) -> Operation:
    ...
    def validate_operation(self, operation: Operation) -> ValidationResult:
        ...
        try:
            ...
            raise SafetyError(
                f"Operation validation pipeline failed: {e}",
    ...
    def _requires_dry_run_enforcement(self, operation: Operation) -> bool:
        ...
        return operation.type in destructive_operations and self.enforce_dry_run_first and not operation.dry_run
    ...
    def _enforce_dry_run_policy(self, operation: Operation) -> ValidationResult:
        ...
        if not operation.dry_run:
            ...
            age_hours = (
                datetime.now() - dry_run_result.summary.get("completed_at", datetime.now())
            ).total_seconds() / 3600
    ...
    def _validate_path_safety(self, operation: Operation) -> ValidationResult:
        ...
        if validation_summary["blocked_paths"] > 0:
            ...
            if blocked_percentage > 25:  # More than 25% blocked
                self.logger.warning(
                    f"High percentage of paths blocked ({blocked_percentage:.1f}%), requiring confirmation"
                )
                return ValidationResult.REQUIRES_CONFIRMATION
    ...
    def execute_safe_operation(self, operation: Operation) -> OperationResult:
        ...
        try:
            ...
            self.logger.info(
                f"Executing operation {operation.id} with manifest {manifest_id} (dry_run={operation.dry_run})"
            )
            ...
            try:
                ...
                self.manifest_system.log_error(
                    manifest_id,
                    e,
                    {
            ...
            raise SafetyError(
                f"Unexpected error during safe operation execution: {e}",
    ...
    def _execute_operation_with_monitoring(self, operation: Operation, manifest_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"processed_items": processed_items, "errors": errors, "success": len(errors) == 0}

        except Exception as e:
            self.logger.error(f"Error during operation execution: {e}")
            raise

    def _finalize_operation_execution(
        self, operation: Operation, manifest_id: str, execution_result: Dict[str, Any], execution_start: datetime
    ) -> OperationResult:
        ...
        if success:
            self.logger.info(
                f"Operation {operation.id} completed successfully: "
                f"{execution_result['processed_items']} items processed in {execution_time:.2f}s"
            )
        else:
            self.logger.warning(
                f"Operation {operation.id} completed with {len(execution_result['errors'])} errors "
            ...
            try:
                ...
                if not allowed:
                    ...
                    self.manifest_system.log_file_action(
                        manifest_id, "file" if path.is_file() else "directory", path, f"blocked: {denial}"
                    )
                    continue
                ...
                if operation.dry_run:
                    ...
                    self.manifest_system.log_file_action(
                        manifest_id, "file" if path.is_file() else "directory", path, "would_delete"
                    )
                else:
                    # Actual deletion
                    if path.is_file():
    ...
    def _execute_analyze_operation(self, operation: Operation, manifest_id: str) -> tuple[int, List[str]]:
        ...
        for path in operation.paths:
            try:
                ...
                self.manifest_system.log_file_action(
                    manifest_id, "file" if path.is_file() else "directory", path, "analyzed"
                )
                processed_items += 1
    ...
    def get_restore_candidates(self) -> List[Dict[str, Any]]:
        ...
        for manifest in manifests:
            ...
            if (
                manifest.get("operation") in ["delete", "clean"]
                and manifest.get("status") == "completed"
                and not manifest.get("options", {}).get("dry_run", True)
            ):
    ...
    def get_safety_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "components_status": {"path_validator": "active", "manifest_system": "active", "process_manager": "active"},
        }
    ...
    def validate_system_safety(self) -> Dict[str, Any]:
        ...
        """
        validation_results = {"overall_status": "safe", "checks": {}, "warnings": [], "errors": []}
