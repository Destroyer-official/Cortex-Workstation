        ...
        try:
            ...
            if self.target_folders and self.include_python:
                ...
                resources = cleaner.scan_caches(
                    target_folders=self.target_folders, include_python_projects=True, keep_recent_days=self.keep_recent
                )
            else:
                # Scan default package manager caches
                resources = cleaner.scan_caches(keep_recent_days=self.keep_recent)
...
class PackageManagerTab(BaseTab):
    ...
    def setup_ui(self):
        ...
        self.pm_scan_button.setStyleSheet(
            "QPushButton { font-weight: bold; background-color: #0078d4; color: white; border-radius: 4px; }"
        )
        ...
        self.pm_cleanup_button.setStyleSheet(
            "QPushButton { font-weight: bold; background-color: #d4860c; color: white; border-radius: 4px; } QPushButton:disabled { background-color: #999; }"
        )
        ...
        self.pm_progress_bar.setStyleSheet(
    ...
    def _on_scan_finished(self, data):
        ...
        self.pm_summary_label.setText(
            f"Found {len(self.pm_resources)} cache locations: {self.format_bytes(total_size)} total"
        )
        if total_size > 0:
