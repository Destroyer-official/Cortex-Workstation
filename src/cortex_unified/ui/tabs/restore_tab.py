...
class RestoreTab(BaseTab):
    ...
    def setup_ui(self):
        ...
        self.quarantine_table.setHorizontalHeaderLabels(
            ["ID", "Path", "Size", "Deleted At", "Backup Path", "Can Restore"]
        )
        ...
        if self.vss_health is not None:
            try:
                ...
                self.vss_health_lbl.setText(
                    f"VSS writers healthy: {report.healthy_writer_count} | " f"failed: {report.failed_writer_count}"
                )
            except Exception as e:
    ...
    def _on_restore_completed(self, results):
        ...
        if errors:
            QMessageBox.warning(
                ...
                f"Restored {restored} items. Emitted {len(errors)} warnings! Check logs.",
            )
            self.logger.warning(f"Restore output constraints: {errors}")
        else:
            QMessageBox.information(
                self, "Restore Completed", f"Successfully extracted and recovered {restored} files safely."
            )
