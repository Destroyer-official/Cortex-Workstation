...
class SchedulerTab(BaseTab):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if success:
                ...
                QMessageBox.critical(
                    self, "Error", "Failed to register OS Task. Ensure you have administrator/root privileges."
                )
    ...
    def create_auto_clean_rules_subtab(self) -> QWidget:
        ...
        apply_btn.setStyleSheet(
            "QPushButton { font-weight: bold; background-color: #2196F3; color: white; padding: 5px; }"
        )
    ...
    def _apply_rules(self):
        ...
        QMessageBox.information(
            self, "Rules Applied", f"System will now passively flush temp boundaries crossing {limit}% disk usage."
        )
