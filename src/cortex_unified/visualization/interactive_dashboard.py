...
try:
    ...
    class px:
        ...
        @staticmethod
        def bar(*args, **kwargs):
            """Return an empty Figure stub for bar-chart calls."""
            return go.Figure()

        @staticmethod
        def pie(*args, **kwargs):
            """Return an empty Figure stub for pie-chart calls."""
            return go.Figure()

    def make_subplots(*args, **kwargs):
        """Return an empty Figure stub for subplot layouts."""
        return go.Figure()
...
class InteractiveDashboard:
    ...
    def _create_empty_dashboard(self) -> go.Figure:
        ...
        fig.update_layout(
            title="Interactive Disk Usage Dashboard", xaxis=dict(visible=False), yaxis=dict(visible=False), height=600
        )
        return fig
    ...
    def _create_treemap_dashboard(self) -> go.Figure:
        ...
        fig.update_layout(title="Interactive Disk Usage TreeMap", height=700)
        return fig
    ...
    def _create_sunburst_dashboard(self) -> go.Figure:
        ...
        fig.update_layout(title="Interactive Disk Usage Sunburst", height=700)
        return fig
    ...
    def _create_side_by_side_dashboard(self) -> go.Figure:
        ...
        fig.update_layout(title="Interactive Disk Usage Dashboard", height=700, showlegend=False)
    ...
    def _create_combined_dashboard(self) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2,
            cols=2,
            subplot_titles=("Disk Usage Overview", "File Type Distribution", "Directory TreeMap", "Size Distribution"),
            specs=[[{"type": "pie"}, {"type": "bar"}], [{"type": "treemap", "colspan": 2}, None]],
        ...
        fig.update_layout(title="Comprehensive Disk Usage Dashboard", height=800, showlegend=True)
    ...
    def _add_disk_usage_pie(self, fig: go.Figure, row: int, col: int):
        ...
        if hasattr(self.analyzer, "disk_usage") and self.analyzer.disk_usage:
            ...
            values = [disk_data.get("used_bytes", 0), disk_data.get("free_bytes", 0)]
    ...
    def _add_file_type_bar(self, fig: go.Figure, row: int, col: int):
        ...
        if hasattr(self.analyzer, "file_type_breakdown") and self.analyzer.file_type_breakdown:
            ...
            sorted_types = sorted(file_types.items(), key=lambda x: x[1].get("size_bytes", 0), reverse=True)[:10]
    ...
    def handle_drill_down(self, path: str) -> go.Figure:
        ...
        if self.analyzer and hasattr(self.analyzer, "__class__"):
            ...
            drill_analyzer = DiskAnalyzer(config=getattr(self.analyzer, "config", None), root_path=path)
    ...
    def handle_context_menu(self, path: str) -> Dict[str, Any]:
        ...
        """
        context_actions = {
            "open_in_explorer": {"label": "Open in File Explorer", "action": "open_explorer", "path": path},
            "drill_down": {"label": "Drill Down", "action": "drill_down", "path": path},
            "exclude_from_scan": {"label": "Exclude from Future Scans", "action": "exclude", "path": path},
            "analyze_separately": {"label": "Analyze Separately", "action": "analyze", "path": path},
        }
