...
class SunburstGenerator:
    ...
    def generate_sunburst(self, max_depth: int = 4) -> go.Figure:
        ...
        if hasattr(self.data, "directory_tree") and self.data.directory_tree:
            sunburst_data = self._convert_directory_tree_to_sunburst_data(self.data.directory_tree, max_depth)
        elif isinstance(self.data, dict):
            sunburst_data = self._convert_directory_tree_to_sunburst_data(self.data, max_depth)
        else:
            fig = go.Figure(
                go.Sunburst(
                    labels=["No Data"],
        ...
        fig = go.Figure(
            go.Sunburst(
                ...
                marker=dict(colors=sunburst_data["colors"], line=dict(color="white", width=2)),
                maxdepth=max_depth,
            )
        )

        fig.update_layout(
            title={"text": "Disk Usage Sunburst Chart", "x": 0.5, "xanchor": "center"},
            font=dict(size=12),
