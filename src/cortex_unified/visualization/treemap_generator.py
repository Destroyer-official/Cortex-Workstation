...
class TreeMapGenerator:
    ...
    def _convert_directory_tree_to_nodes(
        self, tree_data: Dict, max_depth: int = 3, current_depth: int = 0
    ) -> List[TreeMapNode]:
        ...
        for tree in trees:
            ...
            if children_data and current_depth < max_depth - 1:
                node.children = self._convert_directory_tree_to_nodes(children_data, max_depth, current_depth + 1)
    ...
    def generate_treemap(self, max_depth: int = 3) -> go.Figure:
        ...
        """
        if not self.data:
            fig = go.Figure(go.Treemap(labels=["No Data"], parents=[""], values=[1], textinfo="label"))
        ...
        fig = go.Figure(
            go.Treemap(
                ...
                marker=dict(colors=plotly_data["colors"], line=dict(width=2, color="white")),
                maxdepth=max_depth,
                branchvalues="total",
            )
        )
        ...
        # Update layout
        fig.update_layout(
            title={"text": "Disk Usage TreeMap", "x": 0.5, "xanchor": "center"},
            font=dict(size=12),
    ...
    def export_as_image(self, format: str = "png", width: int = 1200, height: int = 800) -> bytes:
        ...
        try:
            ...
            img_bytes = fig.to_image(format=format.lower(), width=width, height=height, engine="kaleido")
    ...
    def export_as_html(self, interactive: bool = True, include_plotlyjs: str = "cdn") -> str:
        ...
        try:
            ...
            html_str = plot(fig, output_type="div", include_plotlyjs=include_plotlyjs, config=config)
