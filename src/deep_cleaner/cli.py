"""Command-line interface for Deep Cleaner."""
...
def clean_empty(
    ...
    # Log startup info
    logger.info(f"Deep Cleaner starting...")
    ...
    try:
        from .performance.resource_throttler import ResourceThrottler
...
@main.command()
@click.option('--clean', is_flag=True, default=False, help='Clean temp files (requires confirmation)')
@click.option('--pattern', multiple=True, help='Only consider files matching this glob pattern (can be used multiple times)')
@click.option('--exclude-pattern', multiple=True, help='Exclude files/directories matching this pattern (can be used multiple times)')
@click.option('--config', type=click.Path(exists=False), help='Path to configuration file')
@click.option('--no-config', is_flag=True, default=False, help='Don\'t load any configuration file')
...
def clean_temp(
    ...
    # Log startup info
    logger.info(f"Finding temporary files...")
    
    try:
        # Create cleaner and find temp files
        cleaner = TempCleaner(config_obj)
        temp_files = cleaner.find_temp_files()
        stats = cleaner.get_stats()
        
        logger.info(f"Found {stats['temp_files_found']} temporary files")
        logger.info(f"Total size: {stats['total_size_human']}")
        
        if temp_files:
            logger.info("Temporary files:")
        ...
        if export:
            import json
            export_data = {
                "temp_files": [str(path) for path in temp_files],
                "stats": stats
            }
            with open(export, 'w') as f:
                json.dump(export_data, f, indent=2)
        ...
        if clean and temp_files:
            logger.info(f"Selected {len(temp_files)} files for cleaning")
            
            if not yes:
                click.confirm(f"Delete {len(temp_files)} temporary files? This action cannot be undone.", abort=True)
            else:
                logger.info("Skipping confirmation due to --yes flag")
            
            # Delete files
            deleter = Deleter(dry_run=False, use_trash=True)
            result = deleter.delete(temp_files, [])
...
def analyze_disk(
    ...
    Examples:
      deep-cleaner analyze-disk                              # Basic analysis
      deep-cleaner analyze-disk --export-treemap tree.html  # Interactive TreeMap
      deep-cleaner analyze-disk --export-sunburst sun.html  # Sunburst chart
      deep-cleaner analyze-disk --max-depth 5               # Deeper analysis
      deep-cleaner analyze-disk --resume-from checkpoint.json  # Resume scan
...
def docker_cleanup(
    ...
    Examples:
      deep-cleaner docker-cleanup                    # Show what would be cleaned
      deep-cleaner docker-cleanup --clean --all     # Clean all Docker resources
      deep-cleaner docker-cleanup --images --clean  # Clean only unused images
      deep-cleaner docker-cleanup --export report.json  # Export findings to JSON
...
def package_cleanup(
    ...
    Examples:
      deep-cleaner package-cleanup                    # Show what would be cleaned
      deep-cleaner package-cleanup --clean --all     # Clean all package managers
      deep-cleaner package-cleanup --pip --clean     # Clean only pip cache
      deep-cleaner package-cleanup --orphaned        # Find orphaned packages
...
def heuristics_scan(
    ...
    Examples:
      deep-cleaner heuristics-scan                           # Scan current directory
      deep-cleaner heuristics-scan --confidence-threshold 0.9  # High confidence only
      deep-cleaner heuristics-scan --scan-registry          # Include registry analysis (Windows)
      deep-cleaner heuristics-scan /path/to/scan            # Scan specific directory
...
@main.command()
@click.option('--images', is_flag=True, default=False, help='Clean unused Docker images')
@click.option('--containers', is_flag=True, default=False, help='Clean stopped Docker containers')
@click.option('--volumes', is_flag=True, default=False, help='Clean unused Docker volumes')
@click.option('--networks', is_flag=True, default=False, help='Clean unused Docker networks')
@click.option('--all', 'clean_all', is_flag=True, default=False, help='Clean all Docker resources')
@click.option('--dry-run', is_flag=True, default=True, help='Show what would be cleaned without actually cleaning (default)')
@click.option('--force', is_flag=True, default=False, help='Actually perform the cleanup')
@click.option('--yes', is_flag=True, default=False, help='Skip confirmation prompts')
@click.option('--verbose', is_flag=True, default=False, help='Enable verbose output')
@click.option('--log-file', type=click.Path(), help='Write logs to file')
@click.option('--json-log', is_flag=True, default=False, help='Output logs in JSON format')
...
def docker_cleanup(
    ...
    """Clean up Docker resources (images, containers, volumes, networks)."""
    ...
    # If no specific resources selected, default to all
    if not any([images, containers, volumes, networks]):
        if clean_all:
            images = containers = volumes = networks = True
        else:
            # Default to images and containers only for safety
            images = containers = True
    ...
    try:
        ...
        if images:
            logger.info("Scanning for unused Docker images...")
            unused_images = cleaner.scan_unused_images()
            all_resources.extend(unused_images)
            logger.info(f"Found {len(unused_images)} unused images")
            
            if verbose and unused_images:
                logger.info("Unused images:")
                for image in unused_images:
                    size_human = cleaner._format_bytes(image.size)
                    status = "DANGLING" if image.is_dangling else "UNUSED"
                    logger.info(f"  [{status}] {image} - {size_human}")
        
        if containers:
            logger.info("Scanning for stopped Docker containers...")
            stopped_containers = cleaner.scan_stopped_containers()
            all_resources.extend(stopped_containers)
            logger.info(f"Found {len(stopped_containers)} stopped containers")
            
            if verbose and stopped_containers:
                logger.info("Stopped containers:")
                for container in stopped_containers:
                    size_human = cleaner._format_bytes(container.size)
                    logger.info(f"  [{container.status.upper()}] {container} - {size_human}")
        
        if volumes:
            logger.info("Scanning for unused Docker volumes...")
            unused_volumes = cleaner.scan_unused_volumes()
            all_resources.extend(unused_volumes)
            logger.info(f"Found {len(unused_volumes)} unused volumes")
            
            if verbose and unused_volumes:
                logger.info("Unused volumes:")
                for volume in unused_volumes:
                    size_human = cleaner._format_bytes(volume.size)
                    logger.info(f"  [ORPHANED] {volume} - {size_human}")
        
        if networks:
            logger.info("Scanning for unused Docker networks...")
            unused_networks = cleaner.scan_unused_networks()
            all_resources.extend(unused_networks)
            logger.info(f"Found {len(unused_networks)} unused networks")
            
            if verbose and unused_networks:
                logger.info("Unused networks:")
                for network in unused_networks:
                    logger.info(f"  [UNUSED] {network}")
        
        # Calculate total space that would be freed
        total_space = sum(getattr(resource, 'size', 0) for resource in all_resources)
        total_space_human = cleaner._format_bytes(total_space)
        
        logger.info(f"Total resources found: {len(all_resources)}")
        logger.info(f"Total space to be freed: {total_space_human}")
        
        if not all_resources:
            logger.info("No Docker resources to clean up")
            return
        ...
        if export:
            ...
            export_data = {
                ...
                "total_size_human": total_space_human
            }
            
            for resource in all_resources:
                resource_data = {
                    "type": type(resource).__name__,
                    "id": getattr(resource, 'id', getattr(resource, 'name', 'unknown')),
        ...
        if not dry_run:
            if not yes:
                click.confirm(
                    f"Clean up {len(all_resources)} Docker resources and free {total_space_human}? "
        ...
        # Perform cleanup
        result = cleaner.cleanup_resources(all_resources, dry_run=dry_run)
        
        # Report results
        if dry_run:
            logger.info(f"Would clean up {result.total_removed} resources:")
        else:
            logger.info(f"Cleaned up {result.total_removed} resources:")
        
        logger.info(f"  Images: {result.images_removed}")
        logger.info(f"  Containers: {result.containers_removed}")
        logger.info(f"  Volumes: {result.volumes_removed}")
        logger.info(f"  Networks: {result.networks_removed}")
        logger.info(f"  Space freed: {cleaner._format_bytes(result.space_freed)}")
...
def list_checkpoints(config, verbose):
    ...
    from .performance import ScanManager
...
def scan_enhanced(
    ...
    # Log startup info
    logger.info(f"Deep Cleaner Enhanced Scan starting...")
    logger.info(f"Target path: {target_path}")
    logger.info(f"Action mode: {action}")
