...
class DriveManager:
    ...
    def _store_credentials(self, credentials: Dict[str, str]) -> None:
        ...
        for server, cred_info in credentials.items():
            try:
                ...
                try:
                    if password:
                        keyring.set_password("deep_cleaner_network", f"{server}:{username}", password)
                except Exception as e:
                    logger.warning(f"Could not store credentials securely: {e}")
    ...
    def _connect_unix_network_drive(self, network_drive: NetworkDrive, credentials: Dict[str, str]) -> None:
        ...
        # This would require more complex implementation depending on the network filesystem
