"""File and directory scanning functionality for Deep Cleaner."""
...
from .utils import is_system_directory, get_file_age_days
from .config import Config
from .performance import ScanManager, ResourceThrottler
...
class Scanner:
    ...
    def _scan_directory(self, dirpath: Path) -> Tuple[bool, List[Path], List[Path]]:
        """Scan a directory and its contents."""
        ...
        for entry in entries:
            try:
                ...
                if entry.is_file():
                    ...
                    is_empty, sub_files, sub_dirs = self._scan_directory(entry)
                    if is_empty:
    ...
    def scan(self, threads: int = 0, checkpoint_id: Optional[str] = None) -> Tuple[List[Path], List[Path]]:
