...
class TestLeftoversScan:
    ...
    def test_scan_json_emits_dicts(self, fake_scan):
        ...
        findings = fake_scan(
            [
                LeftoverFinding(
                    kind="folder", path=r"C:\x\Zeta", size_bytes=2048, score=8, level="VeryGood", reasons=["+4 empty"]
                ),
            ]
        )
        result = CliRunner().invoke(main, ["leftovers", "scan", "ZetaEditor", "--json"])
    ...
    def test_scan_human_output_shows_confidence(self, fake_scan):
        ...
        fake_scan(
            [
                LeftoverFinding(kind="folder", path=r"C:\x\Zeta", size_bytes=1, score=6, level="Good"),
            ]
        )
...
class TestLeftoversClean:
    ...
    def test_dry_run_is_default_and_deletes_nothing(self, fake_scan, tmp_path):
        ...
        target.mkdir()
        fake_scan([LeftoverFinding(kind="folder", path=str(target), size_bytes=5, score=8, level="VeryGood")])
        result = CliRunner().invoke(main, ["leftovers", "clean", "ZetaEditor"])
        assert result.exit_code == 0
        assert "[DRY-RUN]" in result.output
        assert target.exists()  # nothing was touched

    def test_min_level_filters_questionable_by_default(self, fake_scan, tmp_path):
        ...
        fake_scan(
            [
                LeftoverFinding(kind="folder", path=str(low), size_bytes=1, score=1, level="Questionable"),
                LeftoverFinding(kind="folder", path=str(high), size_bytes=2, score=8, level="VeryGood"),
            ]
        )
        result = CliRunner().invoke(main, ["leftovers", "clean", "App", "--json"])
        payload = json.loads(result.output)
    ...
    def test_apply_recycles_and_reports_freed_bytes(self, fake_scan, tmp_path, monkeypatch):
        ...
        target.mkdir()
        fake_scan([LeftoverFinding(kind="folder", path=str(target), size_bytes=4096, score=8, level="VeryGood")])
        ...
        class FakeCleaner:
            ...
            def clean(self, models, create_restore_point=False):
                ...
                return [lc.CleanOutcome(models[0].path, models[0].kind, True, "recycled")]

        monkeypatch.setattr(lc, "LeftoverCleaner", FakeCleaner)
        result = CliRunner().invoke(main, ["leftovers", "clean", "App", "--apply", "--json"])
        payload = json.loads(result.output)
        assert payload["ok"] == 1
        assert payload["recycled_bytes"] == 4096

    def test_apply_failure_exits_nonzero(self, fake_scan, tmp_path, monkeypatch):
        ...
        target.mkdir()
        fake_scan([LeftoverFinding(kind="folder", path=str(target), size_bytes=1, level="VeryGood", score=8)])
        from cortex_unified.system_tools import leftover_cleaner as lc
        ...
        class FailingCleaner:
            ...
            def clean(self, models, create_restore_point=False):
                ...
                Args:
                    ...
                    create_restore_point: The create restore point parameter.
                """
                return [lc.CleanOutcome(models[0].path, models[0].kind, False, "failed", "denied")]
...
class TestLeftoversOrphans:
    ...
    def test_orphans_lists_findings(self, fake_scan):
        ...
        Args:
            fake_scan: The fake scan parameter.
        """
        fake_scan([LeftoverFinding(kind="folder", path=r"C:\PF\Ghost", size_bytes=0, score=6, level="Good")])
