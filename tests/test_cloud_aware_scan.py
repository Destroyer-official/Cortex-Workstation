...
def test_attribute_readers_tolerate_a_posix_stat(tmp_path):
    """Verify attribute readers tolerate a posix stat via winattrs.reparse_tag_of, f.stat, winattrs.attrs_of.
...
def test_describe_explains_special_entries():
    """Verify describe explains special entries via winattrs.describe."""
...
def test_to_dict_reports_cloud_state():
    """Verify to dict reports cloud state via FileEntry, Path, to_dict."""
...
def test_placeholders_can_be_included_on_request(tmp_path, cloud_attrs):
    """Verify placeholders can be included on request via walker.scan, FastWalker, WalkOptions.
...
def test_find_empty_never_offers_a_placeholder_for_deletion(tmp_path, cloud_attrs):
    """Verify find empty never offers a placeholder for deletion via real_empty.touch, online_empty.touch, FastWalker.
...
def test_walker_still_reports_plain_trees_unchanged(tmp_path):
    """Verify walker still reports plain trees unchanged via FastWalker, WalkOptions, scan.
...
def test_on_disk_size_falls_back_when_the_path_is_gone():
    """Verify on disk size falls back when the path is gone via winattrs.on_disk_size."""
