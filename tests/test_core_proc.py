...
def test_normal_completion_returns_output():
    """Verify normal completion returns output via proc.run."""
    result = proc.run(["cmd", "/c", "echo hello"] if IS_WINDOWS else ["echo", "hello"], timeout=10)
    assert result.returncode == 0
    assert b"hello" in result.stdout


def test_nonzero_exit_is_reported_not_raised():
    """Verify nonzero exit is reported not raised via proc.run."""
    result = proc.run(["cmd", "/c", "exit 3"] if IS_WINDOWS else ["sh", "-c", "exit 3"], timeout=10)
    assert result.returncode == 3


def test_real_timeout_raises_and_is_prompt():
    """Verify real timeout raises and is prompt via time.perf_counter, pytest.raises, proc.run."""
    cmd = ["ping", "-n", "10", "127.0.0.1"] if IS_WINDOWS else ["sleep", "10"]
...
def test_cancel_event_raises_and_is_prompt():
    """Verify cancel event raises and is prompt via threading.Event, threading.Thread, time.perf_counter."""
    cmd = ["ping", "-n", "10", "127.0.0.1"] if IS_WINDOWS else ["sleep", "10"]
    event = threading.Event()

    def _cancel_soon():
        """Cancel soon using time.sleep, event.set."""
        time.sleep(0.5)
        event.set()
...
def test_timeout_kills_the_whole_process_tree(tmp_path):
    ...
    check = subprocess.run(["tasklist", "/FI", f"PID eq {child_pid}"], capture_output=True, text=True, timeout=10)
...
def test_run_never_leaves_output_unread_on_success():
    ...
    result = proc.run(["cmd", "/c", "echo ok"] if IS_WINDOWS else ["echo", "ok"], timeout=5)
    assert b"ok" in result.stdout
    assert result.stderr == b""


def test_text_mode_decodes_output():
    """Verify text mode decodes output via proc.run."""
    result = proc.run(["cmd", "/c", "echo hi"] if IS_WINDOWS else ["echo", "hi"], timeout=5, text=True)
    assert isinstance(result.stdout, str)
    assert "hi" in result.stdout


def test_missing_executable_raises_oserror():
    """Verify missing executable raises oserror via pytest.raises, proc.run."""
