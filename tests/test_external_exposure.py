...
def test_lookup_requires_consent_and_global_public_ip():
    ...
    client = ExternalExposureClient("shodan", "secret", transport=lambda *_args: {})
...
def test_shodan_sends_only_selected_ip_and_normalizes_services():
    ...
    def transport(url, headers, timeout):
        ...
        return {
            ...
            "data": [
                {"port": 443, "transport": "tcp", "product": "Caddy", "version": "2.7"},
                {"port": 443, "transport": "tcp", "product": "Caddy", "version": "2.7"},
            ],
        }

    result = ExternalExposureClient("shodan", "api-key", transport=transport).lookup("8.8.8.8", consent=True)
...
def test_censys_credentials_use_header_not_url():
    ...
    result = ExternalExposureClient("censys", "id", "secret", transport).lookup("1.1.1.1", consent=True)
