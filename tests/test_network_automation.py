...
def test_schedule_builds_only_fixed_private_scan_command(monkeypatch):
    ...
    assert command[:4] == ["C:/Python/python.exe", "-m", "cortex_unified.system_tools.network_scan_cli", "--profile"]
    port_index = command.index("--ports")
    assert command[port_index : port_index + 2] == ["--ports", "22,80,8000,8001,8002"]
    assert "--output" in command
    arguments = automation.build_windows_arguments(spec)
    assert arguments[:6] == ["schtasks", "/create", "/f", "/tn", r"\Cortex Cleaner\Network Security Audit", "/tr"]
...
def test_schedule_rejects_public_scope_and_arbitrary_frequency():
    ...
    with pytest.raises(ValueError, match="private LAN"):
        automation.build_scan_command(automation.NetworkSchedule(scopes=("8.8.8.0/24",)))
    with pytest.raises(ValueError, match="frequency"):
        automation.build_scan_command(automation.NetworkSchedule(frequency="startup"))
