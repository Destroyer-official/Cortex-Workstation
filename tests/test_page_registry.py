...
def test_malformed_factory_is_rejected_with_a_clear_message():
    """Verify malformed factory is rejected with a clear message via registry.PageSpec, pytest.raises, bad.load."""
...
def test_grouped_covers_every_page_exactly_once():
    """Verify grouped covers every page exactly once via registry.grouped."""
    seen: list[str] = []
...
def test_by_id_and_group_of_agree_with_pages():
    """Verify by id and group of agree with pages via registry.group_of."""
    for spec in registry.PAGES:
...
def test_default_page_exists_and_is_reachable():
    """Verify default page exists and is reachable."""
