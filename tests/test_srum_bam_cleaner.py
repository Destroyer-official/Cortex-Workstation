...
def test_filetime_conversion():
    ...
    """Verify filetime conversion via SrumBamCleaner._filetime_to_datetime, struct.pack."""
...
def test_srum_query():
    """Verify srum query via SrumBamCleaner, cleaner.query_srum, info.db_path.endswith."""
...
def test_srum_bam_scan():
    """Verify srum bam scan via cleaner.scan, SrumBamCleaner, report.to_dict."""
    cleaner = SrumBamCleaner()
    report = cleaner.scan()
...
def test_clean_bam_empty():
    """Verify clean bam empty via SrumBamCleaner, cleaner.clean_bam_entries."""
    cleaner = SrumBamCleaner()
